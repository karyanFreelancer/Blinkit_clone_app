...
function shouldGrantGestureMove(dir, dx, dy) {
  ...
  if ((dir === "horizontal" || dir === "left" || dir === "right") && absDx > GESTURE_GRANT_THRESHOLD && absDx > absDy) {
    return true;
  }
  if ((dir === "vertical" || dir === "up" || dir === "down") && absDy > GESTURE_GRANT_THRESHOLD && absDy > absDx) {
    return true;
  }
  return false;
}
function useAnimatedDragGesture(options) {
  ...
  var gesture = React.useMemo(function () {
    ...
    var pan = Gesture.Pan().withRef(gestureRef).shouldCancelWhenOutside(false).runOnJS(true);
    if (isHorizontal) {
    ...
    pan.onStart(function () {
      var _onDragStartRef_current;
      setIsDragging(true);
      (_onDragStartRef_current = onDragStartRef.current) === null || _onDragStartRef_current === void 0 ? void 0 : _onDragStartRef_current.call(onDragStartRef);
      ...
      var relevantVelocity = isHorizontal ? Math.abs(event.velocityX / 1e3) : Math.abs(event.velocityY / 1e3);
