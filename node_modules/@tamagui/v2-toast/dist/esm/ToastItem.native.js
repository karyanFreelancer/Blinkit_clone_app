import { jsx as _jsx, jsxs as _jsxs } from "react/jsx-runtime";
import { isWeb } from "@tamagui/constants";
import { styled, useEvent, View } from "@tamagui/core";
import { XStack, YStack } from "@tamagui/stacks";
import { SizableText } from "@tamagui/text";
import * as React from "react";
import { useDragGesture } from "./useDragGesture.native.js";
import { createNativeToast } from "./createNativeToast.native.js";
var TIME_BEFORE_UNMOUNT = 200;
var ToastItemFrame = styled(YStack, {
  name: "ToastItem",
  focusable: true,
  pointerEvents: "auto",
  position: "absolute",
  left: 0,
  right: 0,
  // for stacking animation - default visible state
  opacity: 1,
  scale: 1,
  y: 0,
  x: 0,
  variants: {
    unstyled: {
      false: {
        backgroundColor: "$background",
        borderRadius: "$4",
        paddingHorizontal: "$4",
        paddingVertical: "$3",
        // shadow using elevation for cross-platform
        elevation: "$4",
        shadowColor: "$shadowColor",
        shadowOffset: {
          width: 0,
          height: 4
        },
        shadowOpacity: 0.15,
        shadowRadius: 12,
        borderWidth: 1,
        borderColor: "$borderColor",
        // only show focus outline on keyboard navigation, not on click/tap
        focusVisibleStyle: {
          outlineStyle: "solid",
          outlineWidth: 2,
          outlineColor: "$outlineColor"
        }
      }
    }
  },
  defaultVariants: {
    unstyled: process.env.TAMAGUI_HEADLESS === "1"
  }
});
var ToastItemTitle = styled(SizableText, {
  name: "ToastItemTitle",
  variants: {
    unstyled: {
      false: {
        fontWeight: "600",
        color: "$color",
        size: "$4"
      }
    }
  },
  defaultVariants: {
    unstyled: process.env.TAMAGUI_HEADLESS === "1"
  }
});
var ToastItemDescription = styled(SizableText, {
  name: "ToastItemDescription",
  variants: {
    unstyled: {
      false: {
        color: "$color11",
        size: "$2",
        marginTop: "$1"
      }
    }
  },
  defaultVariants: {
    unstyled: process.env.TAMAGUI_HEADLESS === "1"
  }
});
var ToastCloseButton = styled(XStack, {
  name: "ToastCloseButton",
  render: "button",
  alignItems: "center",
  justifyContent: "center",
  cursor: "pointer",
  variants: {
    unstyled: {
      false: {
        width: 20,
        height: 20,
        borderRadius: "$10",
        backgroundColor: "$color5",
        hoverStyle: {
          backgroundColor: "$color6"
        },
        pressStyle: {
          backgroundColor: "$color7"
        }
      }
    }
  },
  defaultVariants: {
    unstyled: process.env.TAMAGUI_HEADLESS === "1"
  }
});
var ToastActionButton = styled(XStack, {
  name: "ToastActionButton",
  render: "button",
  alignItems: "center",
  justifyContent: "center",
  cursor: "pointer",
  variants: {
    unstyled: {
      false: {
        borderRadius: "$2",
        paddingHorizontal: "$2",
        height: 24,
        backgroundColor: "$color5",
        hoverStyle: {
          backgroundColor: "$color6"
        },
        pressStyle: {
          backgroundColor: "$color7"
        }
      }
    },
    // primary action button style
    primary: {
      true: {
        backgroundColor: "$color12",
        hoverStyle: {
          backgroundColor: "$color11"
        },
        pressStyle: {
          backgroundColor: "$color10"
        }
      }
    }
  },
  defaultVariants: {
    unstyled: process.env.TAMAGUI_HEADLESS === "1"
  }
});
var DefaultCloseIcon = function () {
  return /* @__PURE__ */_jsx(SizableText, {
    size: "$1",
    color: "$color11",
    children: "\u2715"
  });
};
var DefaultSuccessIcon = function () {
  return /* @__PURE__ */_jsx(SizableText, {
    size: "$5",
    color: "$green10",
    children: "\u2713"
  });
};
var DefaultErrorIcon = function () {
  return /* @__PURE__ */_jsx(SizableText, {
    size: "$5",
    color: "$red10",
    children: "\u2715"
  });
};
var DefaultWarningIcon = function () {
  return /* @__PURE__ */_jsx(SizableText, {
    size: "$5",
    color: "$yellow10",
    children: "\u26A0"
  });
};
var DefaultInfoIcon = function () {
  return /* @__PURE__ */_jsx(SizableText, {
    size: "$5",
    color: "$blue10",
    children: "\u2139"
  });
};
var DefaultLoadingIcon = function () {
  return /* @__PURE__ */_jsx(SizableText, {
    size: "$5",
    color: "$color11",
    children: "\u27F3"
  });
};
var ToastItem = /* @__PURE__ */React.memo(function ToastItem2(props) {
  var _heights_;
  var {
    toast,
    index,
    expanded,
    interacting,
    position,
    visibleToasts,
    removeToast,
    heights,
    setHeights,
    duration,
    gap,
    swipeDirection,
    swipeThreshold,
    closeButton,
    icons,
    disableNative,
    burntOptions,
    notificationOptions
  } = props;
  var [mounted, setMounted] = React.useState(false);
  var [removed, setRemoved] = React.useState(false);
  var [swipeOut, setSwipeOut] = React.useState(false);
  var toastRef = React.useRef(null);
  var closeTimerRef = React.useRef(null);
  var closeTimerStartRef = React.useRef(0);
  var lastPauseTimeRef = React.useRef(0);
  var remainingTimeRef = React.useRef(duration);
  var isFront = index === 0;
  var isVisible = index < visibleToasts;
  var _toast_type;
  var toastType = (_toast_type = toast.type) !== null && _toast_type !== void 0 ? _toast_type : "default";
  var dismissible = toast.dismissible !== false;
  var heightIndex = React.useMemo(function () {
    return heights.findIndex(function (h) {
      return h.toastId === toast.id;
    }) || 0;
  }, [heights, toast.id]);
  var toastsHeightBefore = React.useMemo(function () {
    return heights.reduce(function (prev, curr, reducerIndex) {
      if (reducerIndex >= heightIndex) return prev;
      return prev + curr.height;
    }, 0);
  }, [heights, heightIndex]);
  var offset = heightIndex * gap + toastsHeightBefore;
  var [yPosition] = position.split("-");
  var isTop = yPosition === "top";
  React.useEffect(function () {
    if (!disableNative && !isWeb) {
      var titleText = typeof toast.title === "function" ? toast.title() : toast.title;
      var descText = typeof toast.description === "function" ? toast.description() : toast.description;
      if (typeof titleText === "string") {
        createNativeToast(titleText, {
          message: typeof descText === "string" ? descText : void 0,
          duration,
          burntOptions,
          notificationOptions
        });
      }
    }
  }, []);
  React.useEffect(function () {
    setMounted(true);
  }, []);
  React.useEffect(function () {
    if (toast.delete) {
      setRemoved(true);
      setTimeout(function () {
        removeToast(toast);
      }, TIME_BEFORE_UNMOUNT);
    }
  }, [toast.delete, toast, removeToast]);
  var startTimer = React.useCallback(function () {
    if (duration === Number.POSITIVE_INFINITY || toastType === "loading") return;
    closeTimerStartRef.current = Date.now();
    closeTimerRef.current = setTimeout(function () {
      var _toast_onAutoClose;
      (_toast_onAutoClose = toast.onAutoClose) === null || _toast_onAutoClose === void 0 ? void 0 : _toast_onAutoClose.call(toast, toast);
      setRemoved(true);
      setTimeout(function () {
        removeToast(toast);
      }, TIME_BEFORE_UNMOUNT);
    }, remainingTimeRef.current);
  }, [duration, toastType, toast, removeToast]);
  var pauseTimer = useEvent(function () {
    if (closeTimerRef.current) {
      clearTimeout(closeTimerRef.current);
    }
    if (lastPauseTimeRef.current < closeTimerStartRef.current) {
      var elapsed = Date.now() - closeTimerStartRef.current;
      remainingTimeRef.current = Math.max(0, remainingTimeRef.current - elapsed);
    }
    lastPauseTimeRef.current = Date.now();
  });
  var resumeTimer = useEvent(function () {
    startTimer();
  });
  React.useEffect(function () {
    if (expanded || interacting) {
      pauseTimer();
    } else {
      startTimer();
    }
    return function () {
      if (closeTimerRef.current) {
        clearTimeout(closeTimerRef.current);
      }
    };
  }, [expanded, interacting, startTimer]);
  React.useEffect(function () {
    remainingTimeRef.current = duration;
  }, [duration]);
  var {
    dragState,
    gestureHandlers
  } = useDragGesture({
    direction: swipeDirection,
    threshold: swipeThreshold,
    disabled: !dismissible || toastType === "loading",
    onDragStart: pauseTimer,
    onDragEnd: function (dismissed) {
      if (dismissed) {
        var _toast_onDismiss;
        setSwipeOut(true);
        (_toast_onDismiss = toast.onDismiss) === null || _toast_onDismiss === void 0 ? void 0 : _toast_onDismiss.call(toast, toast);
        setRemoved(true);
        setTimeout(function () {
          removeToast(toast);
        }, TIME_BEFORE_UNMOUNT);
      }
    },
    onDragCancel: resumeTimer
  });
  var handleLayout = React.useCallback(function (event) {
    var {
      height
    } = event.nativeEvent.layout;
    setHeights(function (prev) {
      var exists = prev.find(function (h) {
        return h.toastId === toast.id;
      });
      if (exists) {
        return prev.map(function (h) {
          return h.toastId === toast.id ? {
            ...h,
            height
          } : h;
        });
      }
      return [{
        toastId: toast.id,
        height,
        position
      }, ...prev];
    });
  }, [toast.id, position, setHeights]);
  React.useEffect(function () {
    return function () {
      setHeights(function (prev) {
        return prev.filter(function (h) {
          return h.toastId !== toast.id;
        });
      });
    };
  }, [toast.id, setHeights]);
  var handleClose = React.useCallback(function () {
    var _toast_onDismiss;
    if (!dismissible) return;
    (_toast_onDismiss = toast.onDismiss) === null || _toast_onDismiss === void 0 ? void 0 : _toast_onDismiss.call(toast, toast);
    setRemoved(true);
    setTimeout(function () {
      removeToast(toast);
    }, TIME_BEFORE_UNMOUNT);
  }, [dismissible, toast, removeToast]);
  var getIcon = function () {
    if (toast.icon !== void 0) return toast.icon;
    var _icons_success, _icons_error, _icons_warning, _icons_info, _icons_loading;
    var typeIcons = {
      default: null,
      success: (_icons_success = icons === null || icons === void 0 ? void 0 : icons.success) !== null && _icons_success !== void 0 ? _icons_success : /* @__PURE__ */_jsx(DefaultSuccessIcon, {}),
      error: (_icons_error = icons === null || icons === void 0 ? void 0 : icons.error) !== null && _icons_error !== void 0 ? _icons_error : /* @__PURE__ */_jsx(DefaultErrorIcon, {}),
      warning: (_icons_warning = icons === null || icons === void 0 ? void 0 : icons.warning) !== null && _icons_warning !== void 0 ? _icons_warning : /* @__PURE__ */_jsx(DefaultWarningIcon, {}),
      info: (_icons_info = icons === null || icons === void 0 ? void 0 : icons.info) !== null && _icons_info !== void 0 ? _icons_info : /* @__PURE__ */_jsx(DefaultInfoIcon, {}),
      loading: (_icons_loading = icons === null || icons === void 0 ? void 0 : icons.loading) !== null && _icons_loading !== void 0 ? _icons_loading : /* @__PURE__ */_jsx(DefaultLoadingIcon, {})
    };
    return typeIcons[toastType];
  };
  var icon = getIcon();
  var isHorizontalSwipe = swipeDirection === "left" || swipeDirection === "right" || swipeDirection === "horizontal";
  var isVerticalSwipe = swipeDirection === "up" || swipeDirection === "down" || swipeDirection === "vertical";
  var dragOffsetX = isHorizontalSwipe ? dragState.offsetX : 0;
  var dragOffsetY = isVerticalSwipe ? dragState.offsetY : 0;
  var stackScale = !expanded && !isFront ? 1 - index * 0.05 : 1;
  var _heights__height;
  var frontToastHeight = heights.length > 0 ? (_heights__height = (_heights_ = heights[0]) === null || _heights_ === void 0 ? void 0 : _heights_.height) !== null && _heights__height !== void 0 ? _heights__height : 55 : 55;
  var baseOffset = isTop ? offset : -offset;
  var peekVisible = 10;
  var liftPerToast = peekVisible;
  var stackY = expanded ? baseOffset : isFront ? 0 : isTop ? liftPerToast * index :
  // for top position, toasts stack downward
  -liftPerToast * index;
  var computedY = stackY + dragOffsetY;
  var computedX = dragOffsetX;
  var computedScale = stackScale;
  var computedOpacity = 1;
  if (index >= visibleToasts) {
    computedOpacity = 0;
  } else if (!expanded && index === visibleToasts - 1) {
    computedOpacity = 0.5;
  }
  var computedZIndex = visibleToasts - index;
  var computedHeight = !expanded && !isFront ? frontToastHeight : void 0;
  var computedPointerEvents = index >= visibleToasts ? "none" : "auto";
  if (toast.jsx) {
    return toast.jsx;
  }
  var title = typeof toast.title === "function" ? toast.title() : toast.title;
  var description = typeof toast.description === "function" ? toast.description() : toast.description;
  var dataSet = {
    mounted: mounted ? "true" : "false",
    removed: removed ? "true" : "false",
    swipeOut: swipeOut ? "true" : "false",
    visible: isVisible ? "true" : "false",
    front: isFront ? "true" : "false",
    index: String(index),
    type: toastType,
    expanded: expanded ? "true" : "false"
  };
  var gapFillerHeight = expanded ? gap + 1 : 0;
  var _icons_close;
  return /* @__PURE__ */_jsxs(ToastItemFrame, {
    ref: toastRef,
    role: "status",
    "aria-live": "polite",
    "aria-atomic": true,
    tabIndex: 0,
    // @ts-expect-error dataSet is a valid prop for RN Web compatibility
    dataSet,
    "data-expanded": expanded ? "true" : "false",
    onLayout: handleLayout,
    // use Tamagui animation system - disable animation while dragging
    transition: dragState.isDragging ? void 0 : "quick",
    // animation props
    y: computedY,
    x: computedX,
    scale: computedScale,
    opacity: computedOpacity,
    zIndex: computedZIndex,
    height: computedHeight,
    overflow: computedHeight ? "hidden" : void 0,
    pointerEvents: computedPointerEvents,
    // anchor position: top positions anchor at top, bottom positions anchor at bottom
    top: isTop ? 0 : void 0,
    bottom: isTop ? void 0 : 0,
    // transform-origin: scale from bottom for bottom position, top for top position
    // this ensures the stack peek is visible in the correct direction
    ...(isWeb && !isFront && !expanded && {
      style: {
        transformOrigin: isTop ? "top center" : "bottom center"
      }
    }),
    // enter/exit styles for AnimatePresence
    // subtle animations - small y shift + opacity fade
    enterStyle: {
      opacity: 0,
      y: isTop ? -10 : 10,
      scale: 0.95
    },
    exitStyle: {
      opacity: 0,
      // for swipe dismissal, continue in swipe direction with subtle movement
      x: isHorizontalSwipe && swipeOut ? swipeDirection === "left" ? -30 : 30 : 0,
      y: isVerticalSwipe && swipeOut ? swipeDirection === "up" ? -30 : 30 : isTop ? -10 : 10,
      scale: 0.95
    },
    ...gestureHandlers,
    ...(isWeb && {
      onKeyDown: function (event) {
        if (event.key === "Escape" && dismissible) {
          handleClose();
        }
      }
    }),
    children: [
    /* invisible hit area that fills the gap above/below toast when expanded
    this prevents hover state flickering when mouse moves between toasts */
    expanded && gapFillerHeight > 0 && /* @__PURE__ */_jsx(View, {
      position: "absolute",
      left: 0,
      right: 0,
      height: gapFillerHeight,
      pointerEvents: "auto",
      ...(isTop ? {
        top: "100%"
      } :
      // for top position, extend downward
      {
        bottom: "100%"
      })
    }),
    // for bottom position, extend upward
    /* @__PURE__ */
    _jsxs(XStack, {
      alignItems: "flex-start",
      gap: "$3",
      children: [icon && /* @__PURE__ */_jsx(View, {
        flexShrink: 0,
        marginTop: "$0.5",
        children: icon
      }), /* @__PURE__ */_jsxs(YStack, {
        flex: 1,
        gap: "$1",
        children: [title && /* @__PURE__ */_jsx(ToastItemTitle, {
          children: title
        }), description && /* @__PURE__ */_jsx(ToastItemDescription, {
          children: description
        })]
      }), closeButton && dismissible && /* @__PURE__ */_jsx(ToastCloseButton, {
        onPress: handleClose,
        "aria-label": "Close toast",
        children: (_icons_close = icons === null || icons === void 0 ? void 0 : icons.close) !== null && _icons_close !== void 0 ? _icons_close : /* @__PURE__ */_jsx(DefaultCloseIcon, {})
      })]
    }), /* Action buttons */
    (toast.action || toast.cancel) && /* @__PURE__ */_jsxs(XStack, {
      marginTop: "$3",
      gap: "$2",
      justifyContent: "flex-end",
      children: [toast.cancel && /* @__PURE__ */_jsx(ToastActionButton, {
        onPress: function (event) {
          var _toast_cancel_onClick, _toast_cancel;
          (_toast_cancel = toast.cancel) === null || _toast_cancel === void 0 ? void 0 : (_toast_cancel_onClick = _toast_cancel.onClick) === null || _toast_cancel_onClick === void 0 ? void 0 : _toast_cancel_onClick.call(_toast_cancel, event);
          handleClose();
        },
        children: /* @__PURE__ */_jsx(SizableText, {
          size: "$2",
          children: toast.cancel.label
        })
      }), toast.action && /* @__PURE__ */_jsx(ToastActionButton, {
        primary: true,
        onPress: function (event) {
          var _toast_action_onClick, _toast_action;
          (_toast_action = toast.action) === null || _toast_action === void 0 ? void 0 : (_toast_action_onClick = _toast_action.onClick) === null || _toast_action_onClick === void 0 ? void 0 : _toast_action_onClick.call(_toast_action, event);
          if (!event.defaultPrevented) {
            handleClose();
          }
        },
        children: /* @__PURE__ */_jsx(SizableText, {
          size: "$2",
          fontWeight: "600",
          color: "$background",
          children: toast.action.label
        })
      })]
    })]
  });
});
ToastItem.displayName = "ToastItem";
export { ToastItem };
//# sourceMappingURL=ToastItem.native.js.map
